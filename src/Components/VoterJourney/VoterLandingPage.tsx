...
console.log('The current display restaurant:' + displayRestaurant?.restaurantName)
...
const handleClick = (voteType: any, id: any, restaurantName: any) => {
    setVotes((currVotes: any) => {
     ...
     const childRefs: any = useMemo(() => Array(event.restaurantList?.length).fill(0).map(i => React.createRef()), []);
console.log(childRefs);
const swipe = (dir: any, restaurantID: any, restaurantName: any) => {
   
    const cardToBeRemovedIndex  = event.restaurantList.findIndex((rest: any) => rest._id === restaurantID )
    ...
    setEvent((currEvent: any) => {
        ...
        //splice it out
        let restaurantToRemoveIndex = newRestaurantList.findIndex(rest => rest._id === restaurantID)
...
const swiped = (direction: any, restaurantid: string | undefined, restaurantName: string | undefined) => {
    ...
    // if(displayRestaurant) {
      ...
      setEvent((currEvent: any) => {
          ...
          //find restaurant id in restaurant list
          //splice it out
          let restaurantToRemoveIndex = newRestaurantList.findIndex(rest => rest.restaurantID === restaurantid)
          newRestaurantList.splice(restaurantToRemoveIndex,1);
          newEvent.restaurantList = [...newRestaurantList]
          return newEvent;
      })
    ...
    <div>
        ...
        <div className={Boxes['voting-restaurant-display-container']}>
            {event?.restaurantList.map((restaurant: any) => {
                return (
       <div className={Boxes["tindercard-container"]}><TinderCard onSwipe={(dir: any)=>  swiped(dir, restaurant?._id, restaurant?.restaurantName)} onCardLeftScreen={() => onCardLeftScreen('fooBar')} preventSwipe={['right', 'left']}>
           <div className={Boxes["card"]}>
             <p>{restaurant && restaurant.restaurantName}</p>
            <p>
                {restaurant && restaurant.categories.map((category: string) => {
                return `- ${category} - `;
            })}
            </p>

            <img className={Images['restaurant-info-picture']} src={restaurant && restaurant.imageUrl}></img>
            <a href={restaurant && restaurant.url}>Restaurant Link</a>
